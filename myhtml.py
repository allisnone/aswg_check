...
def get_summary_title_html(h3_content='综合评估',now_time='2018-09-24'):
    ...
    summary_title = '<div style="width:33%; text-align:left; font-size: 18px; \
        color: #2A2C30; font-weight:500;">' + '<h3>%s</h3>' % h3_content + '</div> \
        <div style="width:34%; text-align:center;font-size: 15px; color:#2A2C30; \
        vertical-align: middle;">' + '<span> %s </span>' % now_time + '</div>'
...
def get_L1_summary_html(h4_content='安全评估',loader_img='../static/images/loader.png',fail_img=IMAGE_STATUS['fail'],fail_count=0,
    ...
    h4_content_html = '<h4>%s</h4>' % h4_content
    loader_img_html = '<img id="securityGradeLoader" src="%s" style="width:60px; height:60px; margin:12px 0;">' % loader_img
    fail_img_html = '<img src="%s" style="height:16px; width:16px; margin: 26px 6px 8px 0px;">' % fail_img
    fail_count_html = '<p id="FailCountSecurity" style="color:#D82D24; font-size:20px; display:inline; top:13px; position:relative;">%s</p>' % fail_count
    pass_img_html = '<img src="%s" style="height:16px; width:16px; margin: 26px 6px 8px 0px;">' % pass_img
    pass_count_html = '<p id="PassCountSecurity" style="color:#399C1D; font-size:20px; display:inline; top:13px; position:relative;">%s</p>' % pass_count
    pass_rate = 0
    result_class = 'text_primary'
    
    if (fail_count + pass_count)>0:
        pass_rate = round(round(pass_count,2)/(fail_count + pass_count),3)*100
        if pass_rate<50:
    ...
    pass_result = "{0}%".format(str(pass_rate)[:4])
                ...
                <div class="horizontal-align" style="margin: 10px 12px;">
                    <div style="display:flex;">
                        ...
                        <div style="margin: 0 0 0 8%; width: 35%;">
                            ...
                            <span style="display:block; color: #4C4F54; word-wrap: break-word;">Failed</span>
                        </div>
                        <div style="margin: 0 8% 0 0%; width:25%;">
                            ...
                            <span style="display:block; color: #4C4F54;">Passed</span>
                        </div>
                    </div>
                </div>
                <span style="display:None;">
                    <p style="font-size: 11px; color:#77797C;" class="text-center">Apply to Data Protection test</p>
                </span>
            </div>
        </div>
        </div>\n'''.format(head=h4_content,percentage_result=percentage_result,failimg=fail_img,failcount=fail_count,passimg=pass_img,passcount=pass_count)
...
def get_item_html(request_result,url_mapping=URL_MAPPING,image_status=IMAGE_STATUS,proxies={}):
    ...
    row_html = '''<div class="row {text_class}" style="display:flex; background-color:#ffffff;"> 
        <div class="col-sm-1"> 
